...
function parseSemverVersion(value: string): SemverVersion | null {
...
function getIncompatibleVersionMessage(version: string): string {
  return `Incompatible msecrets file version: expected ${SUPPORTED_SECRETS_FILE_MAJOR}.x.x, got ${version}`;
}

function pushIssue(issues: SecretsFileValidationIssue[], issue: SecretsFileValidationIssue) {
  issues.push(issue);
}
...
function pushError(
  ...
  pushIssue(issues, { code, message, path, severity: "error" });
}
...
function pushWarning(
  ...
  pushIssue(issues, { code, message, path, severity: "warning" });
}

function formatIssue(issue: SecretsFileValidationIssue): string {
...
function validateStringArray(
  ...
  const parsedArray = unknownArraySchema.safeParse(value);
  ...
  parsedArray.data.forEach((entry, index) => {
...
function parsePlainObject(
  ...
  const parsedObject = plainObjectSchema.safeParse(value);
...
export function validateSecretsFile(value: unknown): SecretsFileValidationResult {
  ...
  const parsedKeys = unknownArraySchema.safeParse(keysValue);

  if (!parsedKeys.success) {
    pushError(issues, "keys_invalid", "$.keys", "Keys must be an array");
  } else {
    parsedKeys.data.forEach((key, index) => {
...
export function assertValidSecretsFile(value: unknown): asserts value is SecretsFile {
  ...
  const parsedRoot = plainObjectSchema.safeParse(value);

  if (
    incompatibleVersionIssue &&
    parsedRoot.success &&
    typeof parsedRoot.data["version"] === "string"
  ) {
    throw new SecretsFileVersionError(parsedRoot.data["version"]);
  }
