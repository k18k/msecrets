...
import {
  createFixtureFile,
  createKeyAccess,
  createMissingPrivateKeyAccess,
  createTempSecretsPath,
  generateTestKey,
} from "./helpers.ts";
...
test("workspace snapshot rejects invalid secrets file data", async () => {
  const secretsPath = createTempSecretsPath();
  writeFileSync(secretsPath, JSON.stringify({ version: "2.0.0" }));

  await assert.rejects(
    () => getWorkspaceSnapshot(secretsPath, { keyAccess: createKeyAccess([]) }),
    /Secrets file version is required|Expected an array/,
  );
});
...
test("createSecret normalizes environment before assigning an initial value", async () => {
  const ownerA = await generateTestKey("Owner A");
  ...
  await createSecret(
    secretsPath,
    {
      ...
      fingerprint: ownerA.fingerprint,
      name: "SPACED_ENV",
      plaintextValue: "payload",
    },
    { keyAccess },
  );

  const secret = getSecretsFile(secretsPath).secrets["SPACED_ENV"];
  assert.ok(secret?.values["development"]);
  assert.equal(secret.values[" development "], undefined);
});

test("createSecret rejects explicitly supplied empty initial value", async () => {
  const ownerA = await generateTestKey("Owner A");
  const keyAccess = createKeyAccess([ownerA]);
  const secretsPath = createTempSecretsPath();
...
test("shareSecretValue leaves the file unchanged when re-encryption cannot proceed", async () => {
  const ownerA = await generateTestKey("Owner A");
  const ownerB = await generateTestKey("Owner B");
  const ownerC = await generateTestKey("Owner C");
  const fullKeyAccess = createKeyAccess([ownerA, ownerB, ownerC]);
